...
def verify_str_identifier(
    value: str,
    param_name: str = "identifier: Identifier name"
) -> str:
    """Validate Python identifier."""
    return verify_string(
        value,
        pattern_str=None,#r'^[a-zA-Z_][a-zA-Z0-9_]*$',
        strip_whitespace=True,
        allow_empty=False,
        param_name=param_name
    )
...
def verify_param_names(
    value: str | list[str] | tuple, n: int
    ...
    if count != n:
        ValueError(
            f"Expected 'param_name{s}' to have {n} names, got {count}"
        )
