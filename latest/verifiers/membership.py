...
class MembershipValidator(Generic[T]):
    ...
    def validate(
        ...
        items_str = ", ".join(map(str, self.valid_items[:show_max]))
        if len(self.valid_items) > show_max:
            items_str += f", ..., {len(self.valid_items) - show_max} more"
...
def verify_membership_iterable(
    value: Any,
    valid_items: list[str | int | float],
    case_sensitive: bool = True,
    default: str | int | float | None = None,
